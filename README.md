...
Bombon generates CycloneDX v1.7 SBOMs which aim to be compliant with:

- The German [Technical Guideline TR-03183 v2.1.0][] of the Federal Office for
  Information Security (BSI)
- The US [Executive Order 14028][]

If you find that they aren't compliant in any way, please open an issue!

[Technical Guideline TR-03183 v2.1.0]: https://www.bsi.bund.de/SharedDocs/Downloads/EN/BSI/Publications/TechGuidelines/TR03183/BSI-TR-03183-2_v2_1_0.pdf?__blob=publicationFile&v=5
...
Some language ecosystems in Nixpkgs (most notably Rust, Go, and JavaScript)
vendor dependencies. This means that not every dependency is its own derivation
and thus bombon cannot record their information as it does with "normal" Nix
dependencies. However, bombon can automatically read SBOMs generated by other
tools (like `cargo-cyclonedx`, `npm sbom`, or `cyclonedx-gomod`) for the
vendored dependencies from a passthru derivation called `bombonVendoredSbom`.
...
Similarly, the `passthruVendoredSbom.npm` function covers packages that vendor
their npm dependencies with `fetchNpmDeps` and install them with
`npmConfigHook`, such as anything built with `buildNpmPackage`:

```nix
myNpmPackageWithSbom = bombon.passthruVendoredSbom.npm myNpmPackage { inherit pkgs; };
```

The `passthruVendoredSbom.go` function covers packages built with
`buildGoModule`:
...
`passthruVendoredSbom.rust` and `passthruVendoredSbom.npm` also accept
`includeBuildtimeDependencies` as an optional attribute. For
`passthruVendoredSbom.npm` this includes the npm dev dependencies in the
vendored SBOM. `passthruVendoredSbom.go` does not accept it because the SBOM
is generated from the build info embedded in the compiled binaries, which only
covers the modules linked into them.
