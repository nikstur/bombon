...
tools (like `cargo-cyclonedx` or `npm sbom`) for the vendored dependencies from
a passthru derivation called `bombonVendoredSbom`.
...
The `passthruVendoredSbom` functions also accept `includeBuildtimeDependencies`
as an optional attribute. For `passthruVendoredSbom.npm` this includes the npm
dev dependencies in the vendored SBOM.
